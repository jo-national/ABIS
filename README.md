...
GitHub Actions (gratis "robot" hos GitHub) kører hver nat kl. 04:45 UTC
(06:45 dansk sommertid).
pipeline/fetch.py henter skat.dk-siden, finder Excel-linket dynamisk
...
Kun ved ændring: pipeline/parse.py læser filen, validerer struktur og alle
ISIN-koder, skriver data/abis.json og en læsbar rapport
(data/parse_report.txt). Originalfilen arkiveres uændret i data/raw/.
pipeline/build_site.py genererer hele sitet (én side pr. ISIN, søgning,
sitemap) i site/, som udgives på GitHub Pages.
Fejler noget - siden er nede, linket er væk, strukturen er ændret - fejler
